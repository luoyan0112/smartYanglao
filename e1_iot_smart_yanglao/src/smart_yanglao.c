...
//久坐
static bool sedentary_person_present = false;
static bool sedentary_alarm = false;
static uint32_t sedentary_seconds = 0;


static sedentary_state_t g_sedentary_state =
    SEDENTARY_STATE_NO_PERSON;

static uint32_t g_sitting_seconds = 0;
//时间格式化函数
static void format_sitting_time(
    uint32_t seconds,
    char *buffer,
...
void lcd_show_ui(void)
{
    ...
    lcd_show_string(96, 0, "yanglao home", LCD_RED, LCD_WHITE, 32, 0);
    ...
    sprintf(
        sedentary_time,
        "%02u:%02u:%02u",
        sedentary_seconds / 3600,
        (sedentary_seconds % 3600) / 60,
        sedentary_seconds % 60
    );
...
void lcd_set_sedentary_status(
    ...
    uint32_t seconds)
{
    sedentary_person_present = person_present;
    sedentary_alarm = alarm;
    sedentary_seconds = seconds;
}

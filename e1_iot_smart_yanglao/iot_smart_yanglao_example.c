...
#include "smart_yanglao.h"
#include "smart_yanglao_event.h"
#include "su_03t.h"
#include "iot.h"
#include "lcd.h"
#include "picture.h"
...
#define ROUTE_SSID      "w"          // WiFi账号
#define ROUTE_PASSWORD "789456123"       // WiFi密码
    ...
    while (1)
    {
        ...
        if (ret == LOS_OK) {
            ...
            switch (event_info.event)
            {
                ...
                case event_su03t:
                    smart_home_su03t_cmd_process(
                        event_info.data.su03t_data
                    );
                    break;
        ...
        * 人体检测
        */
        bool person_present = get_infrared_state();

        sedentary_monitor_update(person_present);
        
        lcd_set_sedentary_status(
        sedentary_monitor_is_person_present(),
        sedentary_monitor_is_alarm(),
        sedentary_monitor_get_sitting_seconds()
        );

        printf(
            "[SEDENTARY] raw=%d present=%d state=%s time=%u alarm=%d\n",
            person_present,
            sedentary_monitor_is_person_present(),
            sedentary_monitor_get_state_string(),
            sedentary_monitor_get_sitting_seconds(),
            sedentary_monitor_is_alarm()
        );
        ...
        */
        if (mqtt_is_connected())
        {
            ...
            iot_data.person_present =
                sedentary_monitor_is_person_present();

            iot_data.sitting_seconds =
                sedentary_monitor_get_sitting_seconds();

            iot_data.sedentary_alarm =
                sedentary_monitor_is_alarm();

            iot_data.sedentary_status =
                sedentary_monitor_get_state_string();
            ...
            */
            bool state_changed =
                sedentary_monitor_take_state_changed();

            bool report_due =
                sedentary_monitor_take_report_due();
